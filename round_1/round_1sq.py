...
class Trader:
    ...
    def compute_fair_value(self, order_depth: OrderDepth, product: str, traderObject) -> float:
        # Merge of the similar fair_value logic for KELP and SQUID_INK.
        ...
        # Use a trader-specific key based on the product for storing the last price.
        last_price_key = f"{product.lower()}_last_price"
        if mm_ask is None or mm_bid is None:
            mmmid_price = (
                (best_ask + best_bid) / 2
                if traderObject.get(last_price_key) is None
                else traderObject[last_price_key]
            )
        else:
            mmmid_price = (mm_ask + mm_bid) / 2

        if traderObject.get(last_price_key) is not None:
            last_price = traderObject[last_price_key]
            last_returns = (mmmid_price - last_price) / last_price
            reversion_beta = self.params[product]["reversion_beta"]
    ...
    def run(self, state: TradingState):
        ...
        # RAINFOREST_RESIN uses a fixed fair value.
        ...
        # KELP: compute fair value using merged logic.
        if Product.KELP in self.params and Product.KELP in state.order_depths:
            KELP_position = state.position.get(Product.KELP, 0)
            KELP_fair_value = self.compute_fair_value(state.order_depths[Product.KELP], Product.KELP, traderObject)
        ...
        # SQUID_INK: use geometric brownian motion for fair value adjustment.
        if (
            Product.SQUIDINK in self.params and
            Product.SQUIDINK in state.order_depths and
            self.params[Product.SQUIDINK].get("do_trade", False)
        ):
            SQUIDINK_position = state.position.get(Product.SQUIDINK, 0)
            SQUIDINK_fair_value = self.compute_fair_value(state.order_depths[Product.SQUIDINK], Product.SQUIDINK, traderObject)

            # Maintain history of recent SQUID_INK prices.
            ...
            # Geometric Brownian Motion based prediction using log-returns.
            if len(self.squidink_recent_prices) >= 2:
                valid_prices = [price for price in self.squidink_recent_prices if price > 0]
                if len(valid_prices) >= 2:
                    log_returns = [
                        math.log(valid_prices[i+1] / valid_prices[i])
                        for i in range(len(valid_prices) - 1)
                    ]
                    mu = np.mean(log_returns)
                    predicted_price = SQUIDINK_fair_value * math.exp(mu)
                else:
                    predicted_price = SQUIDINK_fair_value
            else:
                predicted_price = SQUIDINK_fair_value

            # Combine the current fair value with the GBM predicted price.
            weight = 0.5
            adjusted_fair_value = weight * SQUIDINK_fair_value + (1 - weight) * predicted_price
