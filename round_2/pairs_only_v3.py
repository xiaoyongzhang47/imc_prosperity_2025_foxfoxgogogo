...
class Trader:
    ...
    def swmid_fair_value(self, order_depth) -> float:
        best_bid = max(order_depth.buy_orders.keys())
        best_ask = min(order_depth.sell_orders.keys())
    ...
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], int, str]:
        ...
        croissant_fair = self.swmid_fair_value(state.order_depths[Product.CROISSANTS])
        jams_fair = self.swmid_fair_value(state.order_depths[Product.JAMS])
        djembes_fair = self.swmid_fair_value(state.order_depths[Product.DJEMBES])
