...
PARAMS = {
    Product.RAINFORESTRESIN: {
        ...
        "take_width": 1,
        "clear_width": 0,
        # For order placement adjustments:
        "disregard_edge": 1,   # Disregard orders within this edge for pennying or joining
        "join_edge": 2,         # Join orders if they fall within this edge
        "default_edge": 4,      # Default edge when no orders available for pennying/joining
    ...
    Product.SQUIDINK: {
        ...
        # Additional parameter for more aggressive trading during sudden drops
...
class Trader:
    ...
    def compute_fair_value(self, product: str, order_depth: OrderDepth, traderObject: dict) -> float:
        # General fair value computation for KELP and SQUID_INK
        if order_depth.sell_orders and order_depth.buy_orders:
            best_ask = min(order_depth.sell_orders.keys())
            best_bid = max(order_depth.buy_orders.keys())
            adverse_volume = self.params[product].get("adverse_volume", 0)
            last_price_key = f"{product.lower()}_last_price"  # e.g. "kelp_last_price", "squidink_last_price"
            filtered_ask = [
            ...
            if mm_ask is None or mm_bid is None:
                mmmid_price = (best_ask + best_bid) / 2 if traderObject.get(last_price_key) is None else traderObject[last_price_key]
            else:
    ...
    def run(self, state: TradingState):
        ...
        if Product.KELP in self.params and Product.KELP in state.order_depths:
            ...
            KELP_fair_value = self.compute_fair_value(Product.KELP, state.order_depths[Product.KELP], traderObject)
        ...
        # SQUID_INK orders with sudden drop & rebound strategy
        if (Product.SQUIDINK in self.params and Product.SQUIDINK in state.order_depths 
            ...
            SQUIDINK_fair_value = self.compute_fair_value(Product.SQUIDINK, state.order_depths[Product.SQUIDINK], traderObject)

            # Maintain history of recent prices for moving average and slope calculation
            ...
            delta = 0.01
            aggressive_delta = self.params[Product.SQUIDINK].get("aggressive_delta", 0.02)

            if slope < -slope_threshold and normalized_deviation < -deviation_threshold:
                # Detected a sudden drop: adjust fairly aggressively to encourage buying on the drop.
                adjusted_fair_value = SQUIDINK_fair_value * (1 - aggressive_delta)
